...
- (*ctx* context.Context, *cardInfo* cardrecord.StandardCard) を引数として、データベースの適切なテーブルに適切な値を挿入する
...
infoInsert/dto/cardrecordの中に、「魔法」や「罠」に応じたDTO定義を準備する・

この際、cardrecord.goに書かれた定義を
