...
func TestFusionMonsterInsert(t *testing.T) {
	t.Run("正常系01: モンスターの挿入", func(t *testing.T) {
		...
		err := tr.ExecTx(ctx, func(q *sqlc_gen.Queries) error {
			...
			target, err := repoFusion.InsertFusionMonster(ctx, monster.CardID)
			if err != nil {
				return fmt.Errorf("error inserting fusion monster: %w", err)
			}
		...
		assert.NotEqual(t, sqlc_gen.FusionMonster{}, insertedMonster)
		assert.Equal(t, testCardID, insertedMonster.CardID)
	})
}

func TestGetFusionMonsterByCardID(t *testing.T) {
	t.Run("正常系01: 既存のモンスターを取得", func(t *testing.T) {
		...
		err := tr.ExecTx(ctx, func(q *sqlc_gen.Queries) error {
			...
			_, err = repoFusion.InsertFusionMonster(ctx, monster.CardID)
			if err != nil {
				return fmt.Errorf("error inserting fusion monster: %w", err)
			}
			return nil
		...
		assert.Equal(t, true, retrievedMonsterResult2.IsFusion)
	})
