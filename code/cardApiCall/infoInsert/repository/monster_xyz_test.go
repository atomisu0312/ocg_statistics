...
func TestXyzMonsterInsert(t *testing.T) {
	t.Run("正常系01: モンスターの挿入", func(t *testing.T) {
		...
		repoXyz := repository.NewXyzMonsterRepository(q)
		...
		var insertedMonster sqlc_gen.XyzMonster
		err := tr.ExecTx(ctx, func(q *sqlc_gen.Queries) error {
			...
			target, err := repoXyz.InsertXyzMonster(ctx, monster.CardID)
			if err != nil {
				return fmt.Errorf("error inserting xyz monster: %w", err)
			}
		...
		assert.NotEqual(t, sqlc_gen.XyzMonster{}, insertedMonster)
		assert.Equal(t, testCardID, insertedMonster.CardID)
	})
}

func TestGetXyzMonsterByCardID(t *testing.T) {
	t.Run("正常系01: 既存のモンスターを取得", func(t *testing.T) {
		...
		testLevel := int32(6) // Using Level for Rank
		...
		err := tr.ExecTx(ctx, func(q *sqlc_gen.Queries) error {
			...
			_, err = repoXyz.InsertXyzMonster(ctx, monster.CardID)
			if err != nil {
				return fmt.Errorf("error inserting xyz monster: %w", err)
			}
			return nil
		...
		// Now retrieve it
		retrievedMonsterResult, err := repoXyz.GetXyzMonsterByCardID(ctx, testCardID)
		assert.NoError(t, err)
		assert.NotEqual(t, cardrecord.XyzMonsterSelectResult{}, retrievedMonsterResult)
	...
	t.Run("異常系01: 存在しないモンスターを取得", func(t *testing.T) {
		...
		ctx := context.Background()
		q := sqlc_gen.New(dbConn)
		repo := repository.NewXyzMonsterRepository(q)

		nonExistentCardID := int64(99999998) // A card ID that should not exist

		_, err := repo.GetXyzMonsterByCardID(ctx, nonExistentCardID)
		assert.Error(t, err)
