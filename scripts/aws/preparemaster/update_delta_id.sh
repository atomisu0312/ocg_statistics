...
# --- Get currentid ---
CURRENT_ID=$(aws ssm get-parameter --name "${FULL_PARAM_NAME_DELTA}" --query 'Parameter.Value' --output text)
if [ $? -ne 0 ]; then
...
# --- Update currentid ---
aws ssm put-parameter --name "${FULL_PARAM_NAME_DELTA}" --value "$NEW_VALUE" --type "String" --overwrite
if [ $? -ne 0 ]; then
  echo "Error: Failed to update deltaid parameter." >&2
  exit 1
fi
